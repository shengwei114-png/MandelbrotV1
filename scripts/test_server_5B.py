...
model = MandelbrotV1ForCausalLM(
    config=config, start_block_idx=0, end_block_idx=20,
    is_Distribution=True, distribution_Type="server")
...
ds = StreamingTextDataset(config, folder="./data_france/france_en.txt",
    ...
    distribution_Type="server", worker_id="server_0", model=model,
...
print("[Server] Ready. Entering loop...")
...
for step, batch in enumerate(dl):
    ...
    print("[Server] Step", step, "shape:", batch.shape, "cache:", past_key_values is not None)
    with torch.no_grad():
    ...
    print("[Server] Step", step, "loss =", loss.item() if loss is not None else "none")
print("[Server] Done.")

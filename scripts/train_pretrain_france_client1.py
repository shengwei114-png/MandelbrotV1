...
class TrainingPerformanceProfiler:
    ...
    def __init__(self, model, config, device='cuda', enable_gpu_monitor=False, grad_accum_steps=1):
        ...
        self._args = None  # 热更新：外部注入args引用，动态读取enable_gpu_monitor
        self.step_metrics = []
        self.epoch_metrics = []
    ...
    def get_gpu_stats(self):
        ...
        if torch.cuda.is_available():
            # 热更新支持：优先从 _args 动态读取，兼容旧代码的 self.enable_gpu_monitor
            _gpu_mon = self._args.enable_gpu_monitor if self._args is not None else self.enable_gpu_monitor
            if _gpu_mon and NVML_AVAILABLE:
                try:
...
class GPUMonitor:
    ...
    def __init__(self, bytes_shm=None, calls_shm=None):
        ...
        # 网络流量精确测量（读取 /proc/net/dev，汇总所有网口含 lo）
...
def parse_args():
    ...
    # runtime
    p.add_argument("--output_dir", type=str, default="../MandelbrotV1/outputs_incremental_pretraining_client", help="Output directory for checkpoints")
    ...
    p.add_argument("--mem_threshold", type=float, default=0.85, help="系统内存阈值（默认0.85）")
    p.add_argument("--vram_threshold", type=float, default=0.75, help="显存阈值（默认0.85）")
    ...
    # p.add_argument("--resume_from_checkpoint", type=str, default="../MandelbrotV1/outputs_incremental_pretraining_client/checkpoint-180900", help="Path to a checkpoint to resume training from (overrides --model_name_or_path)")
    ...
    p.add_argument("--distribution_Type", type=str, default="client", choices=["server", "client","middle_Server","local","incremental_pretrain_full","incremental_pretrain"], help="分布式训练类型(is_Distribution=True有效)：server,middle_Server,client 本地训练类型(is_Distribution=False有效)：local,incremental_pretrain_full,incremental_pretrain")
...
def main():
    ...
    # ===== ★ 必须在模型加载之前初始化通信监控器 ★ =====
    # gRPC stub 在构造时调用 channel.unary_unary() 并缓存 MultiCallable，
    # 如果 mon.start() 在模型加载之后才 patch，已创建的 stub 不受影响。
...
# ==================================================================
    ...
    if args.use_block_output_cache or (args.generate_block_output_cache and not args.train_folder):
        ...
        if ( args.generate_block_output_cache) and args.paired_train_folder:
            ...
            if ddp_enabled and len(_rank_files) > 0 and False:
    ...
    # ── ★ 必须在 DataLoader 创建之前设置共享内存 ★ ──
    # DataLoader 创建时会 fork worker 子进程，只有此时 _comm_bytes_shm 已非 None，
    # worker 中的 global_worker_init_fn 才会执行 gRPC patch
    ...
    if mon is not None:
        # 更新监控器的共享内存引用（合并 DataLoader worker 子进程的通信量）
        mon._bytes_shm = comm_bytes_shm
        mon._calls_shm = comm_calls_shm
        print("✅ 共享内存计数器已关联到通信监控器")

        # 如果 gRPC patch 未生效，或处于 client 模式（gRPC 在 C 扩展内部，Python 层拦截不到），
        # 回退到包装模型 forward 计时
        ...
        if not mon._grpc_patched:
            ...
            print("⚠️ gRPC patch 未生效，已回退到模型 forward 计时方式")
        elif args.is_Distribution and args.distribution_Type == "client":
            mon.wrap_model_forward(model, hidden_size=_hidden_size)
            print("⚠️ Client 模式：gRPC Python patch 无法拦截 C 层调用，已回退到模型 forward 计时")
    ...
    # 初始化权重矩阵秩分析器（支持热更新：可通过hot_reload动态创建）
    rank_analyzer = None
    if args.enable_rank_analysis:
        ...
        print("\n⚪ 权重矩阵秩分析已禁用（支持热更新开启）")
        print(f"   💡 提示: 使用 --enable_rank_analysis 或热更新启用秩分析")

    # 初始化内在维度（ID）分析器（支持热更新：可通过hot_reload动态创建/销毁）
    id_analyzer = None
    if args.enable_id_analysis:
        ...
        print("\n⚪ 内在维度分析已禁用（支持热更新开启/关闭）")
        print(f"   💡 提示: 使用 --enable_id_analysis 或热更新启用内在维度分析")
    ...
    def _log_training_step(global_step, epoch, synced_running_loss, steps_since_last_log, current_non_zero_count_ratio, current_indep_loss_ratio, scheduler, start_time, tb_writer, args, avg_perf, dataset=None, loss_list=None):
        ...
        # 通信开销统计：字节来自 /proc/net/dev（OS 级真实 gRPC 流量），
        # 时长估算 = 区间耗时 × (1 - GPU利用率/100)
        ...
        for batch in dataloader:
            ...
            if  should_log:
                ...
                if mon is not None and mon.active:
                    ...
                    mon.net_snapshot()  # 重置网络流量快照，统计下一个区间
            ...
            global_step += 1
            # 第一个 micro-batch 之后重置网络快照，排除所有延迟初始化流量
